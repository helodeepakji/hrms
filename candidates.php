...
$jobseeker = $conn->prepare("SELECT `users`.* FROM `users` WHERE `is_terminated` = 3 ORDER BY `users`.`name` ASC");
$jobseeker->execute();
$jobseeker = $jobseeker->fetchAll(PDO::FETCH_ASSOC);

$sql = $conn->prepare("SELECT `users`.*, `job`.`job-title` AS `role` FROM `users` JOIN `job` ON `job`.`id` = `users`.`role_id` WHERE `is_terminated` = 2 ORDER BY `users`.`name` ASC");
$sql->execute();
$users = $sql->fetchAll(PDO::FETCH_ASSOC);


$job = $conn->prepare("SELECT * FROM `job` WHERE `status` = 1");
$job->execute();
			...
			<div class="content">
				...
				<div class="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
					...
					<div class="d-flex my-xl-auto right-content align-items-center flex-wrap ">
						...
						<?php if ($roleId == 1 || (in_array('add-candidates', $pageAccessList))) {
						?>
							<div class="mb-2">
								<a href="#" data-bs-toggle="modal" data-bs-target="#add_employee"
									class="btn btn-primary d-flex align-items-center"><i
										class="ti ti-circle-plus me-2"></i>Add Candidates</a>
							</div>
						<?php } ?>
							...
							<div class="card-body d-flex align-items-center justify-content-between">
								<div class="d-flex align-items-center overflow-hidden">
									...
									<div class="ms-2 overflow-hidden">
										<p class="fs-12 fw-medium mb-1 text-truncate">Total Applicants</p>
										<h4><?php echo count($jobseeker) + count($users) ?></h4>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="card flex-fill">
							<div class="card-body d-flex align-items-center justify-content-between">
								<div class="d-flex align-items-center overflow-hidden">
									...
									<div class="ms-2 overflow-hidden">
										<p class="fs-12 fw-medium mb-1 text-truncate">Total Job</p>
										<h4><?php echo count($job) ?></h4>
									</div>
								</div>
							</div>
						</div>
						...
						<div class="card flex-fill">
							<div class="card-body d-flex align-items-center justify-content-between">
								<div class="d-flex align-items-center overflow-hidden">
									...
									<div class="ms-2 overflow-hidden">
										<p class="fs-12 fw-medium mb-1 text-truncate">Job Seeker</p>
						...
						<div class="card flex-fill">
							<div class="card-body d-flex align-items-center justify-content-between">
								<div class="d-flex align-items-center overflow-hidden">
									...
									<div class="ms-2 overflow-hidden">
										<p class="fs-12 fw-medium mb-1 text-truncate">Not Selected</p>
										<h4><?php echo count($jobseeker) ?></h4>
									</div>
								</div>
							</div>
						</div>
							...
							<div class="me-3">
								<select id="selectedRole" class="form-select">
									...
									foreach ($job as $value) {
										echo '  <option value=' . $value['id'] . '>' . ucfirst(str_replace('_', ' ', $value['job-title'])) . '</option>';
									}
									?>
								...
								<tbody>
									<?php foreach ($users as $user) { ?>
										<tr>
											...
											<td>
												<span
													class="badge badge-<?php echo $user['is_terminated'] == 1 ? 'danger' : 'success' ?> d-inline-flex align-items-center badge-xs" data-bs-toggle="modal"
													data-bs-target="#changeStatus" onclick="changeStatus(<?php echo $user['id'] ?> , '<?php echo $user['job-status'] ?>')">
													<i
														class="ti ti-point-filled me-1"></i><?php echo ucfirst($user['job-status']) ?>
												</span>
											</td>
											<td>
												<?php if ($roleId == 1 || (in_array('candidates-action', $pageAccessList))) {
												?>
			...
			<div class="modal-dialog modal-dialog-centered modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<div class="d-flex align-items-center">
							<h4 class="modal-title me-2">Add New Candidates</h4>
						</div>
									...
									<div class="row">
										<div class="col-md-6">
											<div class="mb-3">
												...
												<input type="hidden" name="type" value="addCandidates">
											</div>
										</div>
										<div class="col-md-6">
											<div class="mb-3">
										...
										<div class="col-md-6">
											<div class="mb-3">
												<label class="form-label">Job <span class="text-danger">
														*</span></label>
												<select class="select" name="job_id" required>
													<option>Select</option>
													<?php foreach ($job as $value) {
														echo '<option value="' . $value['id'] . '">' . $value['job-title'] . '</option>';
													} ?>
												</select>
											</div>
										</div>
		...
		<div class="modal fade" id="edit_employee">
			<div class="modal-dialog modal-dialog-centered modal-lg">
				<div class="modal-content">
					<div class="modal-header">
						<div class="d-flex align-items-center">
							<h4 class="modal-title me-2">Edit Candidates</h4>
						</div>
						<button type="button" class="btn-close custom-btn-close" data-bs-dismiss="modal"
							aria-label="Close">
							<i class="ti ti-x"></i>
			...
			<div class="modal-dialog modal-dialog-centered modal-sm">
				<div class="modal-content">
					<div class="modal-body text-center">
						...
						<p class="mb-3">You want to Reject, this cant be undone once you Reject.
						</p>
						<div class="d-flex justify-content-center">
							<a href="javascript:void(0);" class="btn btn-light me-3" data-bs-dismiss="modal">Cancel</a>
							<a id="employeeDelete" class="btn btn-danger">Yes, Reject</a>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="modal-dialog modal-dialog-centered modal-sm">
				<div class="modal-content">
					<div class="modal-body text-center">
						...
						<h4 class="mb-1">Confirm Job Seeker</h4>
						<p class="mb-3">You want to Job Seeker, this cant be undone once you Job Seeker.
						</p>
						<div class="d-flex justify-content-center">
							<a href="javascript:void(0);" class="btn btn-light me-3" data-bs-dismiss="modal">Cancel</a>
							<a id="employeeDelet" class="btn btn-success">Yes, Job Seeker</a>
						</div>
					</div>
				</div>
			</div>
								...
								<div class="row">
									<div class="col-md-6">
										<div class="mb-3">
											...
											<select class="form-control" name="job-status" id="job-status" required>
												<option value="applied">Applied</option>
												<option value="process">Process</option>
												<option value="select">Selected</option>
												<option value="onboard">On Board</option>
											</select>
											<input type="hidden" name="type" value="changeJobStatus">
											<input type="hidden" name="id" id="applier_id">
										</div>
									</div>
